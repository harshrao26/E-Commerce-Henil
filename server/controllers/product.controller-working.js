...
export async function getAllProductsByFilter(request, response) {
    try {
        const page = parseInt(request.body.page) || 1;
        const perPage = parseInt(request.body.perPage) || 10000;
        ...
        const { categories, productSize, rams, weight } = request.body;
...
export async function getProductsByProductSize(request, response) {


    const size = request.body.size;
    const cat = request.body.category
...
export async function getProductsByProductRam(request, response) {


    const ram = request.body.productRam;
...
export async function getProductsByProductWeight(request, response) {


    const weight = request.body.productWeight;
...
export async function filter(request, response) {
    const { catId, subCatId, productRam, size, productWeight, minPrice, maxPrice, rating } = request.body;
    ...
    if (productRam?.length) filter.productRam = { $in: productRam };
    if (size?.length) filter.size = { $in: size };
    if (productWeight?.length) filter.productWeight = { $in: productWeight };
